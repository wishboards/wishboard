...
- **Never Use `continue-on-error` to Tolerate an Expected Non-Zero Exit:** It suppresses *all* failures, not just the expected one, and reports genuine breakage as a green check. Configure the tool not to fail instead — `stryker.config.json` already sets `thresholds.break: null`, so surviving mutants exit 0 and the flag was pure downside. If a step may legitimately fail, gate the specific condition and keep artifact upload on `if: always()`.
